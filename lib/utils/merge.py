...
def merge_template_search_DAB(inp_list, return_search=False, return_template=False):
    """NOTICE: search region related features must be in the last place"""
    _feats = [x["feat"][0] for x in inp_list]
    _masks = [x["mask"][0] for x in inp_list]
    _pos = [x["pos"][0] for x in inp_list]
    seq_dict = {"feat": _feats,
                "mask": _masks,
                "pos": _pos}
    if return_search:
        x = inp_list[-1]
