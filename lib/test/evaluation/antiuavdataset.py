...
class antiUAVDataset(BaseDataset):
    ...
    def __init__(self, split):
        ...
        self.base_path = os.path.join(self.env_settings.uav_path, split)
    ...
    def _read_anno(self, anno_path):
        ...
        return np.array(new_gt) 

    def _construct_sequence(self, sequence_name):
        anno_path = '{}/{}'.format(self.base_path, sequence_name)

        # ground_truth_rect = load_text(str(anno_path), delimiter=',', dtype=np.float64)
        ground_truth_rect = self._read_anno(anno_path)
