...
from lib.test.vot.vot21_utils import *
# from lib.train.dataset.depth_utils import get_rgbd_frame


class vipt(object):
    def __init__(self, tracker_name='', para_name='', run_id=56):
        # create tracker
        ...
        self.tracker = tracker_info.tracker
        print(tracker_name, self.tracker)
...
def run_vot_exp(tracker_name, para_name, ep=180, vis=False, out_conf=False, channel_type='color'):

    torch.set_num_threads(1)
    save_root = os.path.join('', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = vipt(tracker_name=tracker_name, para_name=para_name, run_id=ep)
